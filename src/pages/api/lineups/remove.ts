 ...
 * POST: Elimina la asignación de un jugador de un equipo
 */
export default async function handler(
  ...
  try {
    const { teamId, playerId } = req.body as RemovePlayerRequest;
    ...
    if (result.success) {
      ...
      return res
        .status(500)
        .json({ error: "Error al eliminar al jugador del equipo" });
    }
  } catch (error) {
